...
#
# Changelog v2.0.1:
#   - Added formatted comparison tables to console output
#   - Integrated table formatting from compare_benchmarks_text.sh
#   - Enhanced results display with ASCII tables for bandwidth, IOPS, and latency
#
# Changelog v2.0.0:
...
generate_plots() {
    if [ $GNUPLOT_AVAILABLE -eq 0 ]; then
        [ "$VERBOSE" -ge 1 ] && echo -e "${YELLOW}Cannot generate graphs, gnuplot is not installed.${NC}"
        return
    fi

    [ "$VERBOSE" -ge 1 ] && echo -e "${BLUE}Generating graphs...${NC}"
    ...
    # Print detected devices and their order for verification (only in verbose mode)
    if [ "$VERBOSE" -ge 1 ]; then
...
EOF
    ...
    if gnuplot "$RESULTS_DIR/bandwidth_plot.gnuplot" 2>"$RESULTS_DIR/gnuplot_error.log"; then
        [ "$VERBOSE" -ge 1 ] && echo -e "${GREEN}Bandwidth graph generated successfully${NC}"
    else
        [ "$VERBOSE" -ge 1 ] && echo -e "${RED}Error generating bandwidth graph. See $RESULTS_DIR/gnuplot_error.log${NC}"
    fi

    gnuplot "$RESULTS_DIR/iops_plot.gnuplot" 2>/dev/null
    gnuplot "$RESULTS_DIR/latency_plot.gnuplot" 2>/dev/null

    # Check if graphs were created successfully (only show in verbose mode)
    if [ "$VERBOSE" -ge 1 ]; then
...
generate_json_report() {
    ...
    [ "$VERBOSE" -ge 1 ] && echo -e "${BLUE}Generating JSON report...${NC}"
...
EOF
    ...
    [ "$VERBOSE" -ge 1 ] && echo -e "${GREEN}JSON report generated: $json_file${NC}"
}
...
generate_markdown_report() {
    ...
    [ "$VERBOSE" -ge 1 ] && echo -e "${BLUE}Generating Markdown report...${NC}"
...
EOF
    ...
    [ "$VERBOSE" -ge 1 ] && echo -e "${GREEN}Markdown report generated: $md_file${NC}"
}

# Generate formatted comparison tables for console output
generate_formatted_summary() {
    echo -e "\n${GREEN}===== Benchmark Comparison =====${NC}\n"

    # Calculate max device name length from CSV
    local max_dev_len=15 # Default minimum
    ...
    ((max_dev_len++)) # Add one for spacing
    ...
    # Collect unique devices in order they appear
    local devices=($(awk -F, 'NR>1 {print $1}' "$RESULTS_DIR/bandwidth_results.csv" | awk '!seen[$0]++'))
    ...
    while IFS=, read -r device test value; do
        [ "$device" = "Device" ] && continue
        printf "| %-${max_dev_len}s | %-11.4f |\n" "$device" "$value"
    done < "$RESULTS_DIR/latency_results.csv"
    printf "+-%s-+-------------+\n" "$dashes"

    # Show results location at the end
    echo -e "\n${GREEN}All benchmarks completed. Results in ${RESULTS_DIR}${NC}"

    # Show generated files (only in verbose mode)
    if [ "$VERBOSE" -ge 1 ]; then
...
generate_report() {
    [ "$VERBOSE" -ge 1 ] && echo -e "${BLUE}Generating final report...${NC}"
    ...
    [ "$VERBOSE" -ge 1 ] && echo -e "${GREEN}Report generated: ${REPORT_FILE}${NC}"

    # Show formatted summary tables on screen
    generate_formatted_summary
}
...
main() {
    ...
    # Generate reports (this calls generate_formatted_summary at the end)
